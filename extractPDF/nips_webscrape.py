...
with open('nips_abstracts.csv', 'w') as csv_file:
    ...
    homepage = urllib2.urlopen(homepagestr)
    ...
    for link in soup.find_all('a'):
    	...
    	if 'book' in newlink:
	    	...
	    	yearpage = urllib2.urlopen(homepagestr+newlink)
	    	...
	    	for ylink in ysoup.find_all('a'):
                ...
                if 'paper' in abstractlink and abstractlink not in abstractLinks:
                    abstractpage = urllib2.urlopen(homepagestr+abstractlink)
                    ...
                    full_text = 'NA'
                    try:
                        if pdf_link is not None and checkLink(pdf_link):
                            full_text = pdf_from_url_to_txt(pdf_link)
                    except Exception, e:
                        pass
                    except urllib2.HTTPError:
                        pass
                    writer.writerow({'year': year, 'authors':authors, 'title': title, 'abstract': abstract, 'html_link': abstractlink, 'pdf_link': pdf_link, 'full_text': full_text})
    csv_file.close()
