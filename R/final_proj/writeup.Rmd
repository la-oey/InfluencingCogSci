...
![optional caption text](lda_graphic.png)
...
```{r}
topic.df.50.processed = topic.df.50 %>% transform_authorAbbr()

author_net = getAuthorList(topic.df.50.processed)
...
```{r centrality_graph,  message=FALSE, warning=FALSE, fig.cap="**Fig. ??: Degree centrality has a long tail, and so it must be log transformed for our analysis."}
centrality %>% 
  filter(CM == 'degree') %>% 
