...
test <- coauthor_mats[[20]]
rownames(test) <- author_key
colnames(test) <- author_key
test <- pmax.matr(test)
test %>%
  as.data.frame() %>%
  gather("authorA","prior_publication",1:ncol(.)) %>%
  mutate(authorB=rep(colnames(test),length(colnames(test)))) %>%
  filter(!is.na(prior_publication)) %>%
  filter((authorA == "J Fan" & authorB %in% c('M Frank','Z Chai','N Goodman','R Hawkins','M Sano','K Mukherjee')) | 
           (authorB == "J Fan" & authorA %in% c('M Frank','Z Chai','N Goodman','R Hawkins','M Sano','K Mukherjee')))
...
for(i in 1:19){
  ...
  temp.next <- topic.coauthor.matrices[[i+1]] %>%
    ...
    dplyr::select(-c(prior_publication, topicSim))
  temp.tot <- temp %>%
...
# 
head(topic.coauthor.matrices[[20]])

for(i in 1:20){
  printname = paste0("topicCoauthMatr/topicCoauth",(i+1999),".csv")
  write.csv(topic.coauthor.matrices[[i]], printname)
}
