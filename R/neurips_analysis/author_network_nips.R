setwd("/Users/loey/Desktop/Research/InfluencingCogSci/R/nips_analysis")
library(tidyverse)
byAuthor <- read_csv("nips_byAuthor.csv")
...
edgeList = function(author_net,uniqueEdges = T){
  ...
  for(k in unique(author_net$title)){
    ...
    if(length(temp$author) == 1){
      edges_itt = data.frame(from = temp$author,to = temp$author)
    }
    else{
      edges_itt = getEdges(temp$author)
    }
...
author_net = byAuthor %>% select(title,author,year) %>% unique() #using author instead of authorAbbr
author_net
...
all_nodes = unique(author_net$author) 
all_edges = edgeList(author_net%>%select(title,author)) #do not run this line with full author_net set it will take HOURS load csv 'all_edges.csv' instead
length(all_nodes)
write.csv(all_nodes, "all_nodes.csv")
nrow(all_edges)
write.csv(all_edges, "all_edges.csv")
library(igraph)

for(j in unique(author_net$year)){
  author_net_year = author_net %>% filter(year == j) %>% select(title, author)
  year_nodes= unique(author_net_year$author)
  year_edges = edgeList(author_net_year)
  
  year_nodes=data.frame(id = 1:length(year_nodes), label = year_nodes)
  write.csv(year_nodes, paste0('centrality_byYear/nodes_',j,'.csv'))
  
  year_edges = apply(year_edges, MARGIN = c(1,2), function(x){return(which(year_nodes[2] == x))})
  year_edges = data.frame(year_edges)
  write.csv(year_edges, paste0('centrality_byYear/edges_',j,'.csv'))
  ...
  write.csv(centrality, paste0('centrality_byYear/centrality_',j,'.csv'))
