...
for(i in 2001:2019){
  year_centrality <- read_csv(paste0("centrality_byYear/centrality_",i,".csv")) %>%
    mutate(year=i) %>%
    dplyr::select(-X1)
...
for(i in 1:10){
  ...
  ggsave(paste0("img/mds_byYear/kmeans_all/full_kmeans_",i,".png"))
}
...
ggsave("img/mds_byYear/kmeans_all/total_within.png")
